...
public abstract class AbstractGcomponentDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Integer>>();
	...
	public void sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.STARTED, true);
			setSuspend(true);
