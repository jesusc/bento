 ...
 * code).
 */
public class GcomponentDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private genericity.language.gcomponent.resource.gcomponent.debug.AbstractGcomponentDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Object>>();
	...
	public GcomponentDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = genericity.language.gcomponent.resource.gcomponent.util.GcomponentStringUtil.encode('#', stack);
				genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_VARIABLES)) {
				...
				genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return genericity.language.gcomponent.resource.gcomponent.util.GcomponentStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Object> pair = new genericity.language.gcomponent.resource.gcomponent.util.GcomponentPair<String, Object>(name, value);
	...
	public genericity.language.gcomponent.resource.gcomponent.debug.AbstractGcomponentDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(genericity.language.gcomponent.resource.gcomponent.debug.AbstractGcomponentDebuggable debuggable) {
