...
public class GcomponentDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static class ReferenceCache implements genericity.language.gcomponent.resource.gcomponent.IGcomponentReferenceCache, org.eclipse.emf.common.notify.Adapter {
	...
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, genericity.language.gcomponent.resource.gcomponent.IGcomponentReferenceResolveResult<ReferenceType> result) {
		try {
			...
			if (!enableScoping) {
				root = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.findRootContainer(container);
			}
			while (root != null) {
	...
	private java.util.Set<org.eclipse.emf.ecore.EObject> findReferencedExternalObjects(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.findRootContainer(object);
	...
	private boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, genericity.language.gcomponent.resource.gcomponent.IGcomponentReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					String result = (String) genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.invokeOperation(element, operation);
					names.add(result);
				}
			}
		}
