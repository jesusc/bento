...
public class GcomponentDefaultNameProvider implements genericity.language.gcomponent.resource.gcomponent.IGcomponentNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
