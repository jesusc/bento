...
public class GbindDebugThread extends org.emftext.language.gbind.resource.gbind.debug.GbindDebugElement implements org.eclipse.debug.core.model.IThread, org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener {
	
	private boolean suspended = false;
	private org.emftext.language.gbind.resource.gbind.debug.GbindDebugTarget debugTarget;
	
	public GbindDebugThread(org.emftext.language.gbind.resource.gbind.debug.GbindDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.emftext.language.gbind.resource.gbind.util.GbindStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.emftext.language.gbind.resource.gbind.debug.GbindStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.emftext.language.gbind.resource.gbind.debug.GbindDebugTarget getTarget() {
		return debugTarget;
	}
