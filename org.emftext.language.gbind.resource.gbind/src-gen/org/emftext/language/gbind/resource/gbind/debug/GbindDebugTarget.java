...
public class GbindDebugTarget extends org.emftext.language.gbind.resource.gbind.debug.GbindDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.emftext.language.gbind.resource.gbind.debug.GbindDebugProcess process;
	...
	private org.emftext.language.gbind.resource.gbind.debug.GbindDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener> eventListeners = new java.util.ArrayList<org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener>();
	
	private org.emftext.language.gbind.resource.gbind.debug.GbindDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.emftext.language.gbind.resource.gbind.mopp.GbindMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public GbindDebugTarget(org.emftext.language.gbind.resource.gbind.debug.GbindDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.emftext.language.gbind.resource.gbind.debug.GbindDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.emftext.language.gbind.resource.gbind.debug.GbindDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.emftext.language.gbind.resource.gbind.debug.IGbindDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.emftext.language.gbind.resource.gbind.mopp.GbindMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.emftext.language.gbind.resource.gbind.debug.GbindLineBreakpoint lineBreakpoint = (org.emftext.language.gbind.resource.gbind.debug.GbindLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage message) {
		try {
			if (message.hasType(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.RESUMED)) {
	...
	public org.emftext.language.gbind.resource.gbind.debug.GbindDebugProxy getDebugProxy() {
