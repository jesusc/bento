 ...
 */
public class GbindDebuggableInterpreter<ResultType, ContextType> extends org.emftext.language.gbind.resource.gbind.debug.AbstractGbindDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.emftext.language.gbind.resource.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.emftext.language.gbind.resource.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public GbindDebuggableInterpreter(org.emftext.language.gbind.resource.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.emftext.language.gbind.resource.gbind.IGbindInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.emftext.language.gbind.resource.gbind.util.GbindStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.emftext.language.gbind.resource.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.emftext.language.gbind.resource.gbind.IGbindLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.emftext.language.gbind.resource.gbind.IGbindLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof org.emftext.language.gbind.resource.gbind.IGbindTextResource) {
			org.emftext.language.gbind.resource.gbind.IGbindTextResource textResource = (org.emftext.language.gbind.resource.gbind.IGbindTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.emftext.language.gbind.resource.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.emftext.language.gbind.resource.gbind.util.GbindEObjectUtil.getDepth(current);
		stopCondition = new org.emftext.language.gbind.resource.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.emftext.language.gbind.resource.gbind.util.GbindEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.emftext.language.gbind.resource.gbind.util.GbindEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.emftext.language.gbind.resource.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
