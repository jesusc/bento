...
public abstract class AbstractGbindDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.emftext.language.gbind.resource.gbind.util.GbindPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.emftext.language.gbind.resource.gbind.util.GbindPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			org.emftext.language.gbind.resource.gbind.mopp.GbindPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			org.emftext.language.gbind.resource.gbind.mopp.GbindPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage message = new org.emftext.language.gbind.resource.gbind.debug.GbindDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.emftext.language.gbind.resource.gbind.util.GbindPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.emftext.language.gbind.resource.gbind.util.GbindPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.emftext.language.gbind.resource.gbind.util.GbindPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.emftext.language.gbind.resource.gbind.debug.EGbindDebugMessageTypes.STARTED, true);
			setSuspend(true);
