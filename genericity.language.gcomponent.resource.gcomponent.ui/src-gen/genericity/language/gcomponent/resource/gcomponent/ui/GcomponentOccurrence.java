 ...
 */
public class GcomponentOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "genericity.language.gcomponent.resource.gcomponent.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "genericity.language.gcomponent.resource.gcomponent.ui.occurences.declaration";
	...
	private genericity.language.gcomponent.resource.gcomponent.ui.GcomponentTokenScanner tokenScanner;
	...
	private genericity.language.gcomponent.resource.gcomponent.IGcomponentTextResource textResource;
	 ...
	 */
	public GcomponentOccurrence(genericity.language.gcomponent.resource.gcomponent.IGcomponentTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, genericity.language.gcomponent.resource.gcomponent.ui.GcomponentTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(genericity.language.gcomponent.resource.gcomponent.ui.GcomponentBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, genericity.language.gcomponent.resource.gcomponent.ui.GcomponentPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, genericity.language.gcomponent.resource.gcomponent.ui.GcomponentPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, genericity.language.gcomponent.resource.gcomponent.ui.GcomponentPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == genericity.language.gcomponent.resource.gcomponent.ui.GcomponentPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(genericity.language.gcomponent.resource.gcomponent.ui.GcomponentOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(genericity.language.gcomponent.resource.gcomponent.ui.GcomponentOccurrence.DECLARATION_ANNOTATION_ID);
	}
