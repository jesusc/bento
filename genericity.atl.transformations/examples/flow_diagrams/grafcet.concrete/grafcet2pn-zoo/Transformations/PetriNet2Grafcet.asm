<?xml version = '1.0' encoding = 'ISO-8859-1' ?>
<asm version="1.0" name="0">
	<cp>
		<constant value="PetriNet2Grafcet"/>
		<constant value="links"/>
		<constant value="NTransientLinkSet;"/>
		<constant value="col"/>
		<constant value="J"/>
		<constant value="main"/>
		<constant value="A"/>
		<constant value="OclParametrizedType"/>
		<constant value="#native"/>
		<constant value="Collection"/>
		<constant value="J.setName(S):V"/>
		<constant value="OclSimpleType"/>
		<constant value="OclAny"/>
		<constant value="J.setElementType(J):V"/>
		<constant value="TransientLinkSet"/>
		<constant value="A.__matcher__():V"/>
		<constant value="A.__exec__():V"/>
		<constant value="self"/>
		<constant value="__resolve__"/>
		<constant value="1"/>
		<constant value="J.oclIsKindOf(J):B"/>
		<constant value="18"/>
		<constant value="NTransientLinkSet;.getLinkBySourceElement(S):QNTransientLink;"/>
		<constant value="J.oclIsUndefined():B"/>
		<constant value="15"/>
		<constant value="NTransientLink;.getTargetFromSource(J):J"/>
		<constant value="17"/>
		<constant value="30"/>
		<constant value="Sequence"/>
		<constant value="2"/>
		<constant value="A.__resolve__(J):J"/>
		<constant value="QJ.including(J):QJ"/>
		<constant value="QJ.flatten():QJ"/>
		<constant value="e"/>
		<constant value="value"/>
		<constant value="resolveTemp"/>
		<constant value="S"/>
		<constant value="NTransientLink;.getNamedTargetFromSource(JS):J"/>
		<constant value="name"/>
		<constant value="__matcher__"/>
		<constant value="A.__matchGrafcet():V"/>
		<constant value="A.__matchStep():V"/>
		<constant value="A.__matchTransition():V"/>
		<constant value="A.__matchStepToTransition():V"/>
		<constant value="A.__matchTransitionToStep():V"/>
		<constant value="__exec__"/>
		<constant value="Grafcet"/>
		<constant value="NTransientLinkSet;.getLinksByRule(S):QNTransientLink;"/>
		<constant value="A.__applyGrafcet(NTransientLink;):V"/>
		<constant value="Step"/>
		<constant value="A.__applyStep(NTransientLink;):V"/>
		<constant value="Transition"/>
		<constant value="A.__applyTransition(NTransientLink;):V"/>
		<constant value="StepToTransition"/>
		<constant value="A.__applyStepToTransition(NTransientLink;):V"/>
		<constant value="TransitionToStep"/>
		<constant value="A.__applyTransitionToStep(NTransientLink;):V"/>
		<constant value="__matchGrafcet"/>
		<constant value="PetriNet"/>
		<constant value="IN"/>
		<constant value="MMOF!Classifier;.allInstancesFrom(S):QJ"/>
		<constant value="TransientLink"/>
		<constant value="NTransientLink;.setRule(MATL!Rule;):V"/>
		<constant value="p"/>
		<constant value="NTransientLink;.addSourceElement(SJ):V"/>
		<constant value="g"/>
		<constant value="NTransientLink;.addTargetElement(SJ):V"/>
		<constant value="NTransientLinkSet;.addLink2(NTransientLink;B):V"/>
		<constant value="12:5-18:4"/>
		<constant value="__applyGrafcet"/>
		<constant value="NTransientLink;"/>
		<constant value="NTransientLink;.getSourceElement(S):J"/>
		<constant value="NTransientLink;.getTargetElement(S):J"/>
		<constant value="3"/>
		<constant value="location"/>
		<constant value="elements"/>
		<constant value="arcs"/>
		<constant value="connections"/>
		<constant value="14:16-14:17"/>
		<constant value="14:16-14:26"/>
		<constant value="14:4-14:26"/>
		<constant value="15:12-15:13"/>
		<constant value="15:12-15:18"/>
		<constant value="15:4-15:18"/>
		<constant value="16:16-16:17"/>
		<constant value="16:16-16:26"/>
		<constant value="16:4-16:26"/>
		<constant value="17:19-17:20"/>
		<constant value="17:19-17:25"/>
		<constant value="17:4-17:25"/>
		<constant value="link"/>
		<constant value="__matchStep"/>
		<constant value="Place"/>
		<constant value="29:3-38:4"/>
		<constant value="__applyStep"/>
		<constant value="net"/>
		<constant value="grafcet"/>
		<constant value="isInitial"/>
		<constant value="isActive"/>
		<constant value="incomingArc"/>
		<constant value="incomingConnections"/>
		<constant value="outgoingArc"/>
		<constant value="outgoingConnections"/>
		<constant value="31:16-31:17"/>
		<constant value="31:16-31:26"/>
		<constant value="31:4-31:26"/>
		<constant value="32:12-32:13"/>
		<constant value="32:12-32:18"/>
		<constant value="32:4-32:18"/>
		<constant value="33:15-33:16"/>
		<constant value="33:15-33:20"/>
		<constant value="33:4-33:20"/>
		<constant value="34:17-34:22"/>
		<constant value="34:4-34:22"/>
		<constant value="35:16-35:21"/>
		<constant value="35:4-35:21"/>
		<constant value="36:27-36:28"/>
		<constant value="36:27-36:40"/>
		<constant value="36:4-36:40"/>
		<constant value="37:27-37:28"/>
		<constant value="37:27-37:40"/>
		<constant value="37:4-37:40"/>
		<constant value="__matchTransition"/>
		<constant value="50:3-57:4"/>
		<constant value="__applyTransition"/>
		<constant value="52:16-52:17"/>
		<constant value="52:16-52:26"/>
		<constant value="52:4-52:26"/>
		<constant value="53:12-53:13"/>
		<constant value="53:12-53:18"/>
		<constant value="53:4-53:18"/>
		<constant value="54:15-54:16"/>
		<constant value="54:15-54:20"/>
		<constant value="54:4-54:20"/>
		<constant value="55:27-55:28"/>
		<constant value="55:27-55:40"/>
		<constant value="55:4-55:40"/>
		<constant value="56:27-56:28"/>
		<constant value="56:27-56:40"/>
		<constant value="56:4-56:40"/>
		<constant value="__matchStepToTransition"/>
		<constant value="PlaceToTransition"/>
		<constant value="67:3-74:4"/>
		<constant value="__applyStepToTransition"/>
		<constant value="from"/>
		<constant value="to"/>
		<constant value="69:17-69:18"/>
		<constant value="69:17-69:27"/>
		<constant value="69:5-69:27"/>
		<constant value="70:12-70:13"/>
		<constant value="70:12-70:18"/>
		<constant value="70:4-70:18"/>
		<constant value="71:15-71:16"/>
		<constant value="71:15-71:20"/>
		<constant value="71:4-71:20"/>
		<constant value="72:14-72:15"/>
		<constant value="72:14-72:22"/>
		<constant value="72:4-72:22"/>
		<constant value="73:12-73:13"/>
		<constant value="73:12-73:18"/>
		<constant value="73:4-73:18"/>
		<constant value="__matchTransitionToStep"/>
		<constant value="TransitionToPlace"/>
		<constant value="84:3-91:4"/>
		<constant value="__applyTransitionToStep"/>
		<constant value="86:17-86:18"/>
		<constant value="86:17-86:27"/>
		<constant value="86:5-86:27"/>
		<constant value="87:12-87:13"/>
		<constant value="87:12-87:18"/>
		<constant value="87:4-87:18"/>
		<constant value="88:15-88:16"/>
		<constant value="88:15-88:20"/>
		<constant value="88:4-88:20"/>
		<constant value="89:14-89:15"/>
		<constant value="89:14-89:22"/>
		<constant value="89:4-89:22"/>
		<constant value="90:12-90:13"/>
		<constant value="90:12-90:18"/>
		<constant value="90:4-90:18"/>
	</cp>
	<field name="1" type="2"/>
	<field name="3" type="4"/>
	<operation name="5">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<getasm/>
			<push arg="7"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="9"/>
			<call arg="10"/>
			<dup/>
			<push arg="11"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="12"/>
			<call arg="10"/>
			<call arg="13"/>
			<set arg="3"/>
			<getasm/>
			<push arg="14"/>
			<push arg="8"/>
			<new/>
			<set arg="1"/>
			<getasm/>
			<call arg="15"/>
			<getasm/>
			<call arg="16"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="17" begin="0" end="24"/>
		</localvariabletable>
	</operation>
	<operation name="18">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="4"/>
		</parameters>
		<code>
			<load arg="19"/>
			<getasm/>
			<get arg="3"/>
			<call arg="20"/>
			<if arg="21"/>
			<getasm/>
			<get arg="1"/>
			<load arg="19"/>
			<call arg="22"/>
			<dup/>
			<call arg="23"/>
			<if arg="24"/>
			<load arg="19"/>
			<call arg="25"/>
			<goto arg="26"/>
			<pop/>
			<load arg="19"/>
			<goto arg="27"/>
			<push arg="28"/>
			<push arg="8"/>
			<new/>
			<load arg="19"/>
			<iterate/>
			<store arg="29"/>
			<getasm/>
			<load arg="29"/>
			<call arg="30"/>
			<call arg="31"/>
			<enditerate/>
			<call arg="32"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="2" name="33" begin="23" end="27"/>
			<lve slot="0" name="17" begin="0" end="29"/>
			<lve slot="1" name="34" begin="0" end="29"/>
		</localvariabletable>
	</operation>
	<operation name="35">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="4"/>
			<parameter name="29" type="36"/>
		</parameters>
		<code>
			<getasm/>
			<get arg="1"/>
			<load arg="19"/>
			<call arg="22"/>
			<load arg="19"/>
			<load arg="29"/>
			<call arg="37"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="17" begin="0" end="6"/>
			<lve slot="1" name="34" begin="0" end="6"/>
			<lve slot="2" name="38" begin="0" end="6"/>
		</localvariabletable>
	</operation>
	<operation name="39">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<getasm/>
			<call arg="40"/>
			<getasm/>
			<call arg="41"/>
			<getasm/>
			<call arg="42"/>
			<getasm/>
			<call arg="43"/>
			<getasm/>
			<call arg="44"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="17" begin="0" end="9"/>
		</localvariabletable>
	</operation>
	<operation name="45">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<getasm/>
			<get arg="1"/>
			<push arg="46"/>
			<call arg="47"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<load arg="19"/>
			<call arg="48"/>
			<enditerate/>
			<getasm/>
			<get arg="1"/>
			<push arg="49"/>
			<call arg="47"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<load arg="19"/>
			<call arg="50"/>
			<enditerate/>
			<getasm/>
			<get arg="1"/>
			<push arg="51"/>
			<call arg="47"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<load arg="19"/>
			<call arg="52"/>
			<enditerate/>
			<getasm/>
			<get arg="1"/>
			<push arg="53"/>
			<call arg="47"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<load arg="19"/>
			<call arg="54"/>
			<enditerate/>
			<getasm/>
			<get arg="1"/>
			<push arg="55"/>
			<call arg="47"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<load arg="19"/>
			<call arg="56"/>
			<enditerate/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="33" begin="5" end="8"/>
			<lve slot="1" name="33" begin="15" end="18"/>
			<lve slot="1" name="33" begin="25" end="28"/>
			<lve slot="1" name="33" begin="35" end="38"/>
			<lve slot="1" name="33" begin="45" end="48"/>
			<lve slot="0" name="17" begin="0" end="49"/>
		</localvariabletable>
	</operation>
	<operation name="57">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<push arg="58"/>
			<push arg="58"/>
			<findme/>
			<push arg="59"/>
			<call arg="60"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<get arg="1"/>
			<push arg="61"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="46"/>
			<call arg="62"/>
			<dup/>
			<push arg="63"/>
			<load arg="19"/>
			<call arg="64"/>
			<dup/>
			<push arg="65"/>
			<push arg="46"/>
			<push arg="46"/>
			<new/>
			<call arg="66"/>
			<pusht/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="68" begin="19" end="24"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="63" begin="6" end="26"/>
			<lve slot="0" name="17" begin="0" end="27"/>
		</localvariabletable>
	</operation>
	<operation name="69">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="70"/>
		</parameters>
		<code>
			<load arg="19"/>
			<push arg="63"/>
			<call arg="71"/>
			<store arg="29"/>
			<load arg="19"/>
			<push arg="65"/>
			<call arg="72"/>
			<store arg="73"/>
			<load arg="73"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="74"/>
			<call arg="30"/>
			<set arg="74"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="38"/>
			<call arg="30"/>
			<set arg="38"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="75"/>
			<call arg="30"/>
			<set arg="75"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="76"/>
			<call arg="30"/>
			<set arg="77"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="78" begin="11" end="11"/>
			<lne id="79" begin="11" end="12"/>
			<lne id="80" begin="9" end="14"/>
			<lne id="81" begin="17" end="17"/>
			<lne id="82" begin="17" end="18"/>
			<lne id="83" begin="15" end="20"/>
			<lne id="84" begin="23" end="23"/>
			<lne id="85" begin="23" end="24"/>
			<lne id="86" begin="21" end="26"/>
			<lne id="87" begin="29" end="29"/>
			<lne id="88" begin="29" end="30"/>
			<lne id="89" begin="27" end="32"/>
			<lne id="68" begin="8" end="33"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="3" name="65" begin="7" end="33"/>
			<lve slot="2" name="63" begin="3" end="33"/>
			<lve slot="0" name="17" begin="0" end="33"/>
			<lve slot="1" name="90" begin="0" end="33"/>
		</localvariabletable>
	</operation>
	<operation name="91">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<push arg="92"/>
			<push arg="58"/>
			<findme/>
			<push arg="59"/>
			<call arg="60"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<get arg="1"/>
			<push arg="61"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="49"/>
			<call arg="62"/>
			<dup/>
			<push arg="63"/>
			<load arg="19"/>
			<call arg="64"/>
			<dup/>
			<push arg="65"/>
			<push arg="49"/>
			<push arg="46"/>
			<new/>
			<call arg="66"/>
			<pusht/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="93" begin="19" end="24"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="63" begin="6" end="26"/>
			<lve slot="0" name="17" begin="0" end="27"/>
		</localvariabletable>
	</operation>
	<operation name="94">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="70"/>
		</parameters>
		<code>
			<load arg="19"/>
			<push arg="63"/>
			<call arg="71"/>
			<store arg="29"/>
			<load arg="19"/>
			<push arg="65"/>
			<call arg="72"/>
			<store arg="73"/>
			<load arg="73"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="74"/>
			<call arg="30"/>
			<set arg="74"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="38"/>
			<call arg="30"/>
			<set arg="38"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="95"/>
			<call arg="30"/>
			<set arg="96"/>
			<dup/>
			<getasm/>
			<pushf/>
			<call arg="30"/>
			<set arg="97"/>
			<dup/>
			<getasm/>
			<pushf/>
			<call arg="30"/>
			<set arg="98"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="99"/>
			<call arg="30"/>
			<set arg="100"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="101"/>
			<call arg="30"/>
			<set arg="102"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="103" begin="11" end="11"/>
			<lne id="104" begin="11" end="12"/>
			<lne id="105" begin="9" end="14"/>
			<lne id="106" begin="17" end="17"/>
			<lne id="107" begin="17" end="18"/>
			<lne id="108" begin="15" end="20"/>
			<lne id="109" begin="23" end="23"/>
			<lne id="110" begin="23" end="24"/>
			<lne id="111" begin="21" end="26"/>
			<lne id="112" begin="29" end="29"/>
			<lne id="113" begin="27" end="31"/>
			<lne id="114" begin="34" end="34"/>
			<lne id="115" begin="32" end="36"/>
			<lne id="116" begin="39" end="39"/>
			<lne id="117" begin="39" end="40"/>
			<lne id="118" begin="37" end="42"/>
			<lne id="119" begin="45" end="45"/>
			<lne id="120" begin="45" end="46"/>
			<lne id="121" begin="43" end="48"/>
			<lne id="93" begin="8" end="49"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="3" name="65" begin="7" end="49"/>
			<lve slot="2" name="63" begin="3" end="49"/>
			<lve slot="0" name="17" begin="0" end="49"/>
			<lve slot="1" name="90" begin="0" end="49"/>
		</localvariabletable>
	</operation>
	<operation name="122">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<push arg="51"/>
			<push arg="58"/>
			<findme/>
			<push arg="59"/>
			<call arg="60"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<get arg="1"/>
			<push arg="61"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="51"/>
			<call arg="62"/>
			<dup/>
			<push arg="63"/>
			<load arg="19"/>
			<call arg="64"/>
			<dup/>
			<push arg="65"/>
			<push arg="51"/>
			<push arg="46"/>
			<new/>
			<call arg="66"/>
			<pusht/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="123" begin="19" end="24"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="63" begin="6" end="26"/>
			<lve slot="0" name="17" begin="0" end="27"/>
		</localvariabletable>
	</operation>
	<operation name="124">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="70"/>
		</parameters>
		<code>
			<load arg="19"/>
			<push arg="63"/>
			<call arg="71"/>
			<store arg="29"/>
			<load arg="19"/>
			<push arg="65"/>
			<call arg="72"/>
			<store arg="73"/>
			<load arg="73"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="74"/>
			<call arg="30"/>
			<set arg="74"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="38"/>
			<call arg="30"/>
			<set arg="38"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="95"/>
			<call arg="30"/>
			<set arg="96"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="99"/>
			<call arg="30"/>
			<set arg="100"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="101"/>
			<call arg="30"/>
			<set arg="102"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="125" begin="11" end="11"/>
			<lne id="126" begin="11" end="12"/>
			<lne id="127" begin="9" end="14"/>
			<lne id="128" begin="17" end="17"/>
			<lne id="129" begin="17" end="18"/>
			<lne id="130" begin="15" end="20"/>
			<lne id="131" begin="23" end="23"/>
			<lne id="132" begin="23" end="24"/>
			<lne id="133" begin="21" end="26"/>
			<lne id="134" begin="29" end="29"/>
			<lne id="135" begin="29" end="30"/>
			<lne id="136" begin="27" end="32"/>
			<lne id="137" begin="35" end="35"/>
			<lne id="138" begin="35" end="36"/>
			<lne id="139" begin="33" end="38"/>
			<lne id="123" begin="8" end="39"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="3" name="65" begin="7" end="39"/>
			<lve slot="2" name="63" begin="3" end="39"/>
			<lve slot="0" name="17" begin="0" end="39"/>
			<lve slot="1" name="90" begin="0" end="39"/>
		</localvariabletable>
	</operation>
	<operation name="140">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<push arg="141"/>
			<push arg="58"/>
			<findme/>
			<push arg="59"/>
			<call arg="60"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<get arg="1"/>
			<push arg="61"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="53"/>
			<call arg="62"/>
			<dup/>
			<push arg="63"/>
			<load arg="19"/>
			<call arg="64"/>
			<dup/>
			<push arg="65"/>
			<push arg="53"/>
			<push arg="46"/>
			<new/>
			<call arg="66"/>
			<pusht/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="142" begin="19" end="24"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="63" begin="6" end="26"/>
			<lve slot="0" name="17" begin="0" end="27"/>
		</localvariabletable>
	</operation>
	<operation name="143">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="70"/>
		</parameters>
		<code>
			<load arg="19"/>
			<push arg="63"/>
			<call arg="71"/>
			<store arg="29"/>
			<load arg="19"/>
			<push arg="65"/>
			<call arg="72"/>
			<store arg="73"/>
			<load arg="73"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="74"/>
			<call arg="30"/>
			<set arg="74"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="38"/>
			<call arg="30"/>
			<set arg="38"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="95"/>
			<call arg="30"/>
			<set arg="96"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="144"/>
			<call arg="30"/>
			<set arg="144"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="145"/>
			<call arg="30"/>
			<set arg="145"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="146" begin="11" end="11"/>
			<lne id="147" begin="11" end="12"/>
			<lne id="148" begin="9" end="14"/>
			<lne id="149" begin="17" end="17"/>
			<lne id="150" begin="17" end="18"/>
			<lne id="151" begin="15" end="20"/>
			<lne id="152" begin="23" end="23"/>
			<lne id="153" begin="23" end="24"/>
			<lne id="154" begin="21" end="26"/>
			<lne id="155" begin="29" end="29"/>
			<lne id="156" begin="29" end="30"/>
			<lne id="157" begin="27" end="32"/>
			<lne id="158" begin="35" end="35"/>
			<lne id="159" begin="35" end="36"/>
			<lne id="160" begin="33" end="38"/>
			<lne id="142" begin="8" end="39"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="3" name="65" begin="7" end="39"/>
			<lve slot="2" name="63" begin="3" end="39"/>
			<lve slot="0" name="17" begin="0" end="39"/>
			<lve slot="1" name="90" begin="0" end="39"/>
		</localvariabletable>
	</operation>
	<operation name="161">
		<context type="6"/>
		<parameters>
		</parameters>
		<code>
			<push arg="162"/>
			<push arg="58"/>
			<findme/>
			<push arg="59"/>
			<call arg="60"/>
			<iterate/>
			<store arg="19"/>
			<getasm/>
			<get arg="1"/>
			<push arg="61"/>
			<push arg="8"/>
			<new/>
			<dup/>
			<push arg="55"/>
			<call arg="62"/>
			<dup/>
			<push arg="63"/>
			<load arg="19"/>
			<call arg="64"/>
			<dup/>
			<push arg="65"/>
			<push arg="55"/>
			<push arg="46"/>
			<new/>
			<call arg="66"/>
			<pusht/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="163" begin="19" end="24"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="63" begin="6" end="26"/>
			<lve slot="0" name="17" begin="0" end="27"/>
		</localvariabletable>
	</operation>
	<operation name="164">
		<context type="6"/>
		<parameters>
			<parameter name="19" type="70"/>
		</parameters>
		<code>
			<load arg="19"/>
			<push arg="63"/>
			<call arg="71"/>
			<store arg="29"/>
			<load arg="19"/>
			<push arg="65"/>
			<call arg="72"/>
			<store arg="73"/>
			<load arg="73"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="74"/>
			<call arg="30"/>
			<set arg="74"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="38"/>
			<call arg="30"/>
			<set arg="38"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="95"/>
			<call arg="30"/>
			<set arg="96"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="144"/>
			<call arg="30"/>
			<set arg="144"/>
			<dup/>
			<getasm/>
			<load arg="29"/>
			<get arg="145"/>
			<call arg="30"/>
			<set arg="145"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="165" begin="11" end="11"/>
			<lne id="166" begin="11" end="12"/>
			<lne id="167" begin="9" end="14"/>
			<lne id="168" begin="17" end="17"/>
			<lne id="169" begin="17" end="18"/>
			<lne id="170" begin="15" end="20"/>
			<lne id="171" begin="23" end="23"/>
			<lne id="172" begin="23" end="24"/>
			<lne id="173" begin="21" end="26"/>
			<lne id="174" begin="29" end="29"/>
			<lne id="175" begin="29" end="30"/>
			<lne id="176" begin="27" end="32"/>
			<lne id="177" begin="35" end="35"/>
			<lne id="178" begin="35" end="36"/>
			<lne id="179" begin="33" end="38"/>
			<lne id="163" begin="8" end="39"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="3" name="65" begin="7" end="39"/>
			<lve slot="2" name="63" begin="3" end="39"/>
			<lve slot="0" name="17" begin="0" end="39"/>
			<lve slot="1" name="90" begin="0" end="39"/>
		</localvariabletable>
	</operation>
</asm>
