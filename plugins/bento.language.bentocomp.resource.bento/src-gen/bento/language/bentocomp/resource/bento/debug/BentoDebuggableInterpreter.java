 ...
 */
public class BentoDebuggableInterpreter<ResultType, ContextType> extends bento.language.bentocomp.resource.bento.debug.AbstractBentoDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private bento.language.bentocomp.resource.bento.util.AbstractBentoInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private bento.language.bentocomp.resource.bento.IBentoCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public BentoDebuggableInterpreter(bento.language.bentocomp.resource.bento.util.AbstractBentoInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new bento.language.bentocomp.resource.bento.IBentoInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = bento.language.bentocomp.resource.bento.util.BentoStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public bento.language.bentocomp.resource.bento.util.AbstractBentoInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		bento.language.bentocomp.resource.bento.IBentoLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private bento.language.bentocomp.resource.bento.IBentoLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		bento.language.bentocomp.resource.bento.IBentoCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = bento.language.bentocomp.resource.bento.util.BentoEObjectUtil.getDepth(current);
		stopCondition = new bento.language.bentocomp.resource.bento.IBentoCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = bento.language.bentocomp.resource.bento.util.BentoEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = bento.language.bentocomp.resource.bento.util.BentoEObjectUtil.getDepth(current) - 1;
		stopCondition = new bento.language.bentocomp.resource.bento.IBentoCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
