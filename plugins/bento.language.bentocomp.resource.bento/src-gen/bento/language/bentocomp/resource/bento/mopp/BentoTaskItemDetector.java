...
/**
 * The BentoTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * BentoTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class BentoTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<bento.language.bentocomp.resource.bento.mopp.BentoTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<bento.language.bentocomp.resource.bento.mopp.BentoTaskItem> foundItems = new java.util.ArrayList<bento.language.bentocomp.resource.bento.mopp.BentoTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new bento.language.bentocomp.resource.bento.mopp.BentoTaskItem(keyword, message, localLine, offset, end));
