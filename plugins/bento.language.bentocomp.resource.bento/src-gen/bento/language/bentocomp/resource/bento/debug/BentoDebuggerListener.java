 ...
 * code).
 */
public class BentoDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private bento.language.bentocomp.resource.bento.debug.AbstractBentoDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, bento.language.bentocomp.resource.bento.util.BentoPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, bento.language.bentocomp.resource.bento.util.BentoPair<String, Object>>();
	...
	public BentoDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		bento.language.bentocomp.resource.bento.debug.BentoDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = bento.language.bentocomp.resource.bento.util.BentoStringUtil.encode('#', stack);
				bento.language.bentocomp.resource.bento.debug.BentoDebugMessage message = new bento.language.bentocomp.resource.bento.debug.BentoDebugMessage(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				bento.language.bentocomp.resource.bento.debug.BentoDebugMessage message = new bento.language.bentocomp.resource.bento.debug.BentoDebugMessage(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_VARIABLES)) {
				...
				bento.language.bentocomp.resource.bento.debug.BentoDebugMessage message = new bento.language.bentocomp.resource.bento.debug.BentoDebugMessage(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return bento.language.bentocomp.resource.bento.util.BentoStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		bento.language.bentocomp.resource.bento.util.BentoPair<String, Object> pair = new bento.language.bentocomp.resource.bento.util.BentoPair<String, Object>(name, value);
	...
	public bento.language.bentocomp.resource.bento.debug.AbstractBentoDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(bento.language.bentocomp.resource.bento.debug.AbstractBentoDebuggable debuggable) {
