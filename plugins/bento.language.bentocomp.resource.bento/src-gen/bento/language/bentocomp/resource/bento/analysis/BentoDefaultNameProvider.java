...
public class BentoDefaultNameProvider implements bento.language.bentocomp.resource.bento.IBentoNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = bento.language.bentocomp.resource.bento.util.BentoEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
