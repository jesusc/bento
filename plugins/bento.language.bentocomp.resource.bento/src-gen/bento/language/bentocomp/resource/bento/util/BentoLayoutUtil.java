...
public class BentoLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		bento.language.bentocomp.resource.bento.grammar.BentoSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), bento.language.bentocomp.resource.bento.grammar.BentoGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		bento.language.bentocomp.resource.bento.grammar.BentoSyntaxElement syntaxElement = bento.language.bentocomp.resource.bento.grammar.BentoGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof bento.language.bentocomp.resource.bento.grammar.BentoPlaceholder) {
			bento.language.bentocomp.resource.bento.grammar.BentoPlaceholder placeholder = (bento.language.bentocomp.resource.bento.grammar.BentoPlaceholder) syntaxElement;
			bento.language.bentocomp.resource.bento.IBentoTokenResolver tokenResolver = new bento.language.bentocomp.resource.bento.mopp.BentoTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			bento.language.bentocomp.resource.bento.IBentoTokenResolveResult result = new bento.language.bentocomp.resource.bento.mopp.BentoTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public bento.language.bentocomp.resource.bento.mopp.BentoLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
