 ...
 * contains
 * <code>bento.language.bentocomp.resource.bento.IBentoTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class BentoMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = bento.language.bentocomp.resource.bento.mopp.BentoPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<bento.language.bentocomp.resource.bento.IBentoCommand<Object>> commands = new java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(bento.language.bentocomp.resource.bento.IBentoCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<bento.language.bentocomp.resource.bento.IBentoCommand<Object>> commandsToProcess = new java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (bento.language.bentocomp.resource.bento.IBentoCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, bento.language.bentocomp.resource.bento.IBentoTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final bento.language.bentocomp.resource.bento.IBentoTextDiagnostic diagnostic) {
		final bento.language.bentocomp.resource.bento.IBentoProblem problem = diagnostic.getProblem();
		bento.language.bentocomp.resource.bento.BentoEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new bento.language.bentocomp.resource.bento.IBentoCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == bento.language.bentocomp.resource.bento.BentoEProblemSeverity.ERROR) {
					...
					bento.language.bentocomp.resource.bento.IBentoTextDiagnostic textDiagnostic = (bento.language.bentocomp.resource.bento.IBentoTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof bento.language.bentocomp.resource.bento.mopp.BentoResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((bento.language.bentocomp.resource.bento.mopp.BentoResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<bento.language.bentocomp.resource.bento.IBentoQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, bento.language.bentocomp.resource.bento.util.BentoStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (bento.language.bentocomp.resource.bento.BentoEProblemType nextType : bento.language.bentocomp.resource.bento.BentoEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, bento.language.bentocomp.resource.bento.BentoEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(bento.language.bentocomp.resource.bento.BentoEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(bento.language.bentocomp.resource.bento.BentoEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new bento.language.bentocomp.resource.bento.util.BentoRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new bento.language.bentocomp.resource.bento.IBentoCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					bento.language.bentocomp.resource.bento.mopp.BentoPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
