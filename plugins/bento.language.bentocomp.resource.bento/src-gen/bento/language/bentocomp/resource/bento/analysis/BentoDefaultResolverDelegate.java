...
public class BentoDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation metaInformation = new bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation();
	
	private bento.language.bentocomp.resource.bento.IBentoNameProvider nameProvider = metaInformation.createNameProvider();
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, bento.language.bentocomp.resource.bento.IBentoReferenceResolveResult<ReferenceType> result) {
		try {
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, bento.language.bentocomp.resource.bento.IBentoReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	protected boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, bento.language.bentocomp.resource.bento.IBentoReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean tryToResolveIdentifierInGenModelRegistry(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, bento.language.bentocomp.resource.bento.IBentoReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, bento.language.bentocomp.resource.bento.IBentoReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new bento.language.bentocomp.resource.bento.mopp.BentoChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, bento.language.bentocomp.resource.bento.IBentoTextResource resource) {
	...
	protected bento.language.bentocomp.resource.bento.IBentoReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = org.eclipse.emf.ecore.util.EcoreUtil.getRootContainer(object);
		org.eclipse.emf.common.notify.Adapter adapter = bento.language.bentocomp.resource.bento.util.BentoEObjectUtil.getEAdapter(root, bento.language.bentocomp.resource.bento.analysis.BentoReferenceCache.class);
		bento.language.bentocomp.resource.bento.analysis.BentoReferenceCache cache = bento.language.bentocomp.resource.bento.util.BentoCastUtil.cast(adapter);
		if (cache != null) {
			return cache;
		} else {
			// cache does not exist. create a new one.
			cache = new bento.language.bentocomp.resource.bento.analysis.BentoReferenceCache(nameProvider);
	...
	protected boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (bento.language.bentocomp.resource.bento.util.BentoStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
