...
public abstract class AbstractBentoDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<bento.language.bentocomp.resource.bento.util.BentoPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<bento.language.bentocomp.resource.bento.util.BentoPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new bento.language.bentocomp.resource.bento.util.BentoRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new bento.language.bentocomp.resource.bento.util.BentoRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			bento.language.bentocomp.resource.bento.debug.BentoDebugMessage message = new bento.language.bentocomp.resource.bento.debug.BentoDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				bento.language.bentocomp.resource.bento.util.BentoPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new bento.language.bentocomp.resource.bento.util.BentoPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new bento.language.bentocomp.resource.bento.util.BentoPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(bento.language.bentocomp.resource.bento.debug.EBentoDebugMessageTypes.STARTED, true);
			setSuspend(true);
