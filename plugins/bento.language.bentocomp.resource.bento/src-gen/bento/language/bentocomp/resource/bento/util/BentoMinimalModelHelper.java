 ...
 */
public class BentoMinimalModelHelper {
	
	private final static bento.language.bentocomp.resource.bento.util.BentoEClassUtil eClassUtil = new bento.language.bentocomp.resource.bento.util.BentoEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + bento.language.bentocomp.resource.bento.util.BentoStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = bento.language.bentocomp.resource.bento.util.BentoListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + bento.language.bentocomp.resource.bento.util.BentoStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = bento.language.bentocomp.resource.bento.util.BentoListUtil.castListUnchecked(value);
