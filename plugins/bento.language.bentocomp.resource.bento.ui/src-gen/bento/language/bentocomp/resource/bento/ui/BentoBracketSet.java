...
public class BentoBracketSet {
	...
	private java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements bento.language.bentocomp.resource.bento.IBentoBracketPair {
	 ...
	 */
	private class ClosingListener implements bento.language.bentocomp.resource.bento.ui.IBentoBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public BentoBracketSet(bento.language.bentocomp.resource.bento.ui.BentoEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoBracketPair>();
	 ...
	 */
	public bento.language.bentocomp.resource.bento.IBentoBracketPair getBracketPair(String opening, String closing) {
		for (bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair : bracketPairs) {
	...
	public bento.language.bentocomp.resource.bento.IBentoBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public bento.language.bentocomp.resource.bento.IBentoBracketPair remove(String opening, String closing) {
		for (bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
