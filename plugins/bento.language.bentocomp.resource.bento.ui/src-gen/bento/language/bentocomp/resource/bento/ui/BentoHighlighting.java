 ...
 */
public class BentoHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static bento.language.bentocomp.resource.bento.ui.BentoPositionHelper positionHelper = new bento.language.bentocomp.resource.bento.ui.BentoPositionHelper();
	...
	private bento.language.bentocomp.resource.bento.ui.BentoTokenScanner scanner;
	...
	private bento.language.bentocomp.resource.bento.ui.BentoEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private bento.language.bentocomp.resource.bento.ui.BentoOccurrence occurrence;
	private bento.language.bentocomp.resource.bento.ui.BentoBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, bento.language.bentocomp.resource.bento.IBentoBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public BentoHighlighting(bento.language.bentocomp.resource.bento.IBentoTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, bento.language.bentocomp.resource.bento.ui.BentoColorManager colorManager, bento.language.bentocomp.resource.bento.ui.BentoEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new bento.language.bentocomp.resource.bento.ui.BentoTokenScanner(textResource, colorManager);
		occurrence = new bento.language.bentocomp.resource.bento.ui.BentoOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new bento.language.bentocomp.resource.bento.ui.BentoBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new bento.language.bentocomp.resource.bento.ui.BentoEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
