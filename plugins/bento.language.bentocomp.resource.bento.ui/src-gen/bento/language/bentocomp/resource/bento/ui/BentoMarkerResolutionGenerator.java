...
public class BentoMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(bento.language.bentocomp.resource.bento.IBentoOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof bento.language.bentocomp.resource.bento.mopp.BentoResource) {
					bento.language.bentocomp.resource.bento.mopp.BentoResource customResource = (bento.language.bentocomp.resource.bento.mopp.BentoResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<bento.language.bentocomp.resource.bento.IBentoQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final bento.language.bentocomp.resource.bento.IBentoQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<bento.language.bentocomp.resource.bento.IBentoQuickFix> getQuickFixes(bento.language.bentocomp.resource.bento.IBentoTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					bento.language.bentocomp.resource.bento.IBentoQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
