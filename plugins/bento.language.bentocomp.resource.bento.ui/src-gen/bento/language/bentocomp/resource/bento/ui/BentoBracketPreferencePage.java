 ...
 * </ul>
 */
public class BentoBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation().getSyntaxName();
	
	private bento.language.bentocomp.resource.bento.ui.BentoBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public BentoBracketPreferencePage() {
		super();
		
		bento.language.bentocomp.resource.bento.IBentoMetaInformation metaInformation = new bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new bento.language.bentocomp.resource.bento.ui.BentoBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					bento.language.bentocomp.resource.bento.IBentoBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + bento.language.bentocomp.resource.bento.ui.BentoPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof bento.language.bentocomp.resource.bento.ui.BentoEditor) {
			((bento.language.bentocomp.resource.bento.ui.BentoEditor) editor).invalidateTextRepresentation();
		}
	}
