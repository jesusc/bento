...
/**
 * A text editor for 'bento' models.
 * <p>
 * This editor has id
 * <code>bento.language.bentocomp.resource.bento.ui.BentoEditor</code>
 * The editor's context menu has id
 * <code>bento.language.bentocomp.resource.bento.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>bento.language.bentocomp.resource.bento.EditorRuler</code>.
 * </p>
 */
public class BentoEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, bento.language.bentocomp.resource.bento.IBentoResourceProvider, bento.language.bentocomp.resource.bento.ui.IBentoBracketHandlerProvider, bento.language.bentocomp.resource.bento.ui.IBentoAnnotationModelProvider {
	
	private bento.language.bentocomp.resource.bento.ui.BentoHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private bento.language.bentocomp.resource.bento.ui.BentoCodeFoldingManager codeFoldingManager;
	private bento.language.bentocomp.resource.bento.ui.BentoBackgroundParsingStrategy bgParsingStrategy = new bento.language.bentocomp.resource.bento.ui.BentoBackgroundParsingStrategy();
	private java.util.Collection<bento.language.bentocomp.resource.bento.IBentoBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<bento.language.bentocomp.resource.bento.IBentoBackgroundParsingListener>();
	private bento.language.bentocomp.resource.bento.ui.BentoColorManager colorManager = new bento.language.bentocomp.resource.bento.ui.BentoColorManager();
	private bento.language.bentocomp.resource.bento.ui.BentoOutlinePage outlinePage;
	...
	private bento.language.bentocomp.resource.bento.ui.BentoPropertySheetPage propertySheetPage;
	...
	private bento.language.bentocomp.resource.bento.ui.IBentoBracketHandler bracketHandler;
	...
	public BentoEditor() {
		super();
		setSourceViewerConfiguration(new bento.language.bentocomp.resource.bento.ui.BentoSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), BentoEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								bento.language.bentocomp.resource.bento.IBentoTextResource currentResource = getResource();
				...
				bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("bento.language.bentocomp.resource.bento.EditorContext");
		setRulerContextMenuId("bento.language.bentocomp.resource.bento.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new bento.language.bentocomp.resource.bento.ui.BentoHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new bento.language.bentocomp.resource.bento.ui.BentoCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		bento.language.bentocomp.resource.bento.mopp.BentoNature.activate(inputFile.getProject());
		...
		bento.language.bentocomp.resource.bento.IBentoTextResource loadedResource = (bento.language.bentocomp.resource.bento.IBentoTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				bento.language.bentocomp.resource.bento.IBentoTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof bento.language.bentocomp.resource.bento.IBentoTextResource) {
					setResource((bento.language.bentocomp.resource.bento.IBentoTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the BentoEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(bento.language.bentocomp.resource.bento.IBentoTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new bento.language.bentocomp.resource.bento.ui.BentoOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new bento.language.bentocomp.resource.bento.ui.BentoPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			bento.language.bentocomp.resource.bento.IBentoTextResource textResource = (bento.language.bentocomp.resource.bento.IBentoTextResource) element.eResource();
			bento.language.bentocomp.resource.bento.IBentoLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			bento.language.bentocomp.resource.bento.IBentoTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				bento.language.bentocomp.resource.bento.IBentoTextToken token = lexer.getNextToken();
			...
			bento.language.bentocomp.resource.bento.ui.BentoUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!BentoEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(bento.language.bentocomp.resource.bento.IBentoBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (bento.language.bentocomp.resource.bento.IBentoBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public bento.language.bentocomp.resource.bento.ui.IBentoBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(bento.language.bentocomp.resource.bento.ui.IBentoBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof bento.language.bentocomp.resource.bento.IBentoTextResource)) {
					return false;
				}
				bento.language.bentocomp.resource.bento.IBentoTextResource textResource = (bento.language.bentocomp.resource.bento.IBentoTextResource) resource;
				bento.language.bentocomp.resource.bento.IBentoLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
