 ...
 */
public class BentoOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "bento.language.bentocomp.resource.bento.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "bento.language.bentocomp.resource.bento.ui.occurences.declaration";
	...
	private bento.language.bentocomp.resource.bento.ui.BentoTokenScanner tokenScanner;
	...
	private bento.language.bentocomp.resource.bento.IBentoTextResource textResource;
	 ...
	 */
	public BentoOccurrence(bento.language.bentocomp.resource.bento.IBentoTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, bento.language.bentocomp.resource.bento.ui.BentoTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new bento.language.bentocomp.resource.bento.mopp.BentoMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(bento.language.bentocomp.resource.bento.ui.BentoBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, bento.language.bentocomp.resource.bento.ui.BentoPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == bento.language.bentocomp.resource.bento.ui.BentoPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(bento.language.bentocomp.resource.bento.ui.BentoOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(bento.language.bentocomp.resource.bento.ui.BentoOccurrence.DECLARATION_ANNOTATION_ID);
	}
