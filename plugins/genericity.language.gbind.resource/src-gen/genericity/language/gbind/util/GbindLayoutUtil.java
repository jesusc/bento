...
public class GbindLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<genericity.language.gbind.mopp.GbindLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			genericity.language.gbind.mopp.GbindLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				genericity.language.gbind.mopp.GbindLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(genericity.language.gbind.mopp.GbindLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		genericity.language.gbind.grammar.GbindSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), genericity.language.gbind.grammar.GbindGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public genericity.language.gbind.mopp.GbindLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		genericity.language.gbind.grammar.GbindSyntaxElement syntaxElement = genericity.language.gbind.grammar.GbindGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof genericity.language.gbind.grammar.GbindPlaceholder) {
			genericity.language.gbind.grammar.GbindPlaceholder placeholder = (genericity.language.gbind.grammar.GbindPlaceholder) syntaxElement;
			genericity.language.gbind.IGbindTokenResolver tokenResolver = new genericity.language.gbind.mopp.GbindTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			genericity.language.gbind.IGbindTokenResolveResult result = new genericity.language.gbind.mopp.GbindTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new genericity.language.gbind.mopp.GbindLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public genericity.language.gbind.mopp.GbindLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
