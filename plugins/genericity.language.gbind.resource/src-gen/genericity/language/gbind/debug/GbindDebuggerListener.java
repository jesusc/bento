...
public class GbindDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private genericity.language.gbind.debug.AbstractGbindDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, genericity.language.gbind.util.GbindPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, genericity.language.gbind.util.GbindPair<String, Object>>();
	...
	private void runDebugger() throws java.io.IOException {
		...
		genericity.language.gbind.debug.GbindDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = genericity.language.gbind.util.GbindStringUtil.encode('#', stack);
				genericity.language.gbind.debug.GbindDebugMessage message = new genericity.language.gbind.debug.GbindDebugMessage(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				genericity.language.gbind.debug.GbindDebugMessage message = new genericity.language.gbind.debug.GbindDebugMessage(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_VARIABLES)) {
				...
				genericity.language.gbind.debug.GbindDebugMessage message = new genericity.language.gbind.debug.GbindDebugMessage(genericity.language.gbind.debug.EGbindDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return genericity.language.gbind.util.GbindStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		genericity.language.gbind.util.GbindPair<String, Object> pair = new genericity.language.gbind.util.GbindPair<String, Object>(name, value);
	...
	public genericity.language.gbind.debug.AbstractGbindDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(genericity.language.gbind.debug.AbstractGbindDebuggable debuggable) {
