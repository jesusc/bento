...
public class GbindDebugThread extends genericity.language.gbind.debug.GbindDebugElement implements org.eclipse.debug.core.model.IThread, genericity.language.gbind.debug.IGbindDebugEventListener {
	
	private boolean suspended = false;
	private genericity.language.gbind.debug.GbindDebugTarget debugTarget;
	
	public GbindDebugThread(genericity.language.gbind.debug.GbindDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			genericity.language.gbind.debug.GbindDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = genericity.language.gbind.util.GbindStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new genericity.language.gbind.debug.GbindStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public genericity.language.gbind.debug.GbindDebugTarget getTarget() {
		return debugTarget;
	}
