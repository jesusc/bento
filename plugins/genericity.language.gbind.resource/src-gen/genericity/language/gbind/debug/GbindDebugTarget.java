...
public class GbindDebugTarget extends genericity.language.gbind.debug.GbindDebugElement implements org.eclipse.debug.core.model.IDebugTarget, genericity.language.gbind.debug.IGbindDebugEventListener {
	
	/**
	 * associated system process
	 */
	private genericity.language.gbind.debug.GbindDebugProcess process;
	...
	private genericity.language.gbind.debug.GbindDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<genericity.language.gbind.debug.IGbindDebugEventListener> eventListeners = new java.util.ArrayList<genericity.language.gbind.debug.IGbindDebugEventListener>();
	
	private genericity.language.gbind.debug.GbindDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new genericity.language.gbind.mopp.GbindMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				genericity.language.gbind.debug.GbindDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(genericity.language.gbind.debug.GbindDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((genericity.language.gbind.debug.IGbindDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public GbindDebugTarget(genericity.language.gbind.debug.GbindDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new genericity.language.gbind.debug.GbindDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new genericity.language.gbind.debug.GbindDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(genericity.language.gbind.debug.IGbindDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(genericity.language.gbind.debug.IGbindDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new genericity.language.gbind.mopp.GbindMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					genericity.language.gbind.debug.GbindLineBreakpoint lineBreakpoint = (genericity.language.gbind.debug.GbindLineBreakpoint) breakpoint;
	...
	public void handleMessage(genericity.language.gbind.debug.GbindDebugMessage message) {
		try {
			if (message.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(genericity.language.gbind.debug.EGbindDebugMessageTypes.RESUMED)) {
	...
	public genericity.language.gbind.debug.GbindDebugProxy getDebugProxy() {
