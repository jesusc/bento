 ...
 */
public class GbindDebuggableInterpreter<ResultType, ContextType> extends genericity.language.gbind.debug.AbstractGbindDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private genericity.language.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private genericity.language.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public GbindDebuggableInterpreter(genericity.language.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new genericity.language.gbind.IGbindInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = genericity.language.gbind.util.GbindStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public genericity.language.gbind.util.AbstractGbindInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		genericity.language.gbind.IGbindLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private genericity.language.gbind.IGbindLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		genericity.language.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = genericity.language.gbind.util.GbindEObjectUtil.getDepth(current);
		stopCondition = new genericity.language.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = genericity.language.gbind.util.GbindEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = genericity.language.gbind.util.GbindEObjectUtil.getDepth(current) - 1;
		stopCondition = new genericity.language.gbind.IGbindCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
