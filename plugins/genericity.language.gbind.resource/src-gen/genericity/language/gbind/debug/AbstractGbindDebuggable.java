...
public abstract class AbstractGbindDebuggable {
	 ...
	 */
	private java.util.List<genericity.language.gbind.util.GbindPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<genericity.language.gbind.util.GbindPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new genericity.language.gbind.util.GbindRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new genericity.language.gbind.util.GbindRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			genericity.language.gbind.debug.GbindDebugMessage message = new genericity.language.gbind.debug.GbindDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				genericity.language.gbind.util.GbindPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new genericity.language.gbind.util.GbindPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new genericity.language.gbind.util.GbindPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(genericity.language.gbind.debug.EGbindDebugMessageTypes.STARTED, true);
			setSuspend(true);
