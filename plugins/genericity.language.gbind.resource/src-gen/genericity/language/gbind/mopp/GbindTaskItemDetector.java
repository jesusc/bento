...
/**
 * The GbindTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * GbindTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class GbindTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<genericity.language.gbind.mopp.GbindTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<genericity.language.gbind.mopp.GbindTaskItem> foundItems = new java.util.ArrayList<genericity.language.gbind.mopp.GbindTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new genericity.language.gbind.mopp.GbindTaskItem(keyword, message, localLine, offset, end));
