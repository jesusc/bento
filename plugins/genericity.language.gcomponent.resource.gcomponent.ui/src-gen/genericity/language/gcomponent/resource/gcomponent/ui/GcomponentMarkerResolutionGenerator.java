...
public class GcomponentMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(genericity.language.gcomponent.resource.gcomponent.IGcomponentOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentResource) {
					genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentResource customResource = (genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<genericity.language.gcomponent.resource.gcomponent.IGcomponentQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final genericity.language.gcomponent.resource.gcomponent.IGcomponentQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									genericity.language.gcomponent.resource.gcomponent.ui.GcomponentUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			genericity.language.gcomponent.resource.gcomponent.ui.GcomponentUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<genericity.language.gcomponent.resource.gcomponent.IGcomponentQuickFix> getQuickFixes(genericity.language.gcomponent.resource.gcomponent.IGcomponentTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					genericity.language.gcomponent.resource.gcomponent.IGcomponentQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
