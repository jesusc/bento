 ...
 */
public class GcomponentDebuggableInterpreter<ResultType, ContextType> extends genericity.language.gcomponent.resource.gcomponent.debug.AbstractGcomponentDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private genericity.language.gcomponent.resource.gcomponent.util.AbstractGcomponentInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private genericity.language.gcomponent.resource.gcomponent.IGcomponentCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public GcomponentDebuggableInterpreter(genericity.language.gcomponent.resource.gcomponent.util.AbstractGcomponentInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new genericity.language.gcomponent.resource.gcomponent.IGcomponentInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = genericity.language.gcomponent.resource.gcomponent.util.GcomponentStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public genericity.language.gcomponent.resource.gcomponent.util.AbstractGcomponentInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		genericity.language.gcomponent.resource.gcomponent.IGcomponentLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private genericity.language.gcomponent.resource.gcomponent.IGcomponentLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		genericity.language.gcomponent.resource.gcomponent.IGcomponentCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.getDepth(current);
		stopCondition = new genericity.language.gcomponent.resource.gcomponent.IGcomponentCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = genericity.language.gcomponent.resource.gcomponent.util.GcomponentEObjectUtil.getDepth(current) - 1;
		stopCondition = new genericity.language.gcomponent.resource.gcomponent.IGcomponentCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
