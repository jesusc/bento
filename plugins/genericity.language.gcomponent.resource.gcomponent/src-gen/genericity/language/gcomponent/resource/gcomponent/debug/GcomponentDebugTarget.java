...
public class GcomponentDebugTarget extends genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugElement implements org.eclipse.debug.core.model.IDebugTarget, genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener {
	
	/**
	 * associated system process
	 */
	private genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugProcess process;
	...
	private genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener> eventListeners = new java.util.ArrayList<genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener>();
	
	private genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public GcomponentDebugTarget(genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					genericity.language.gcomponent.resource.gcomponent.debug.GcomponentLineBreakpoint lineBreakpoint = (genericity.language.gcomponent.resource.gcomponent.debug.GcomponentLineBreakpoint) breakpoint;
	...
	public void handleMessage(genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage message) {
		try {
			if (message.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(genericity.language.gcomponent.resource.gcomponent.debug.EGcomponentDebugMessageTypes.RESUMED)) {
	...
	public genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugProxy getDebugProxy() {
