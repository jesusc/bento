...
/**
 * The GcomponentTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The GcomponentTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class GcomponentTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentTaskItem> foundItems = new java.util.ArrayList<genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new genericity.language.gcomponent.resource.gcomponent.mopp.GcomponentTaskItem(keyword, message, localLine, offset, end));
