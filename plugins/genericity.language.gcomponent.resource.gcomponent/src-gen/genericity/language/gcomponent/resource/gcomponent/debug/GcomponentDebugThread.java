...
public class GcomponentDebugThread extends genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugElement implements org.eclipse.debug.core.model.IThread, genericity.language.gcomponent.resource.gcomponent.debug.IGcomponentDebugEventListener {
	
	private boolean suspended = false;
	private genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugTarget debugTarget;
	
	public GcomponentDebugThread(genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = genericity.language.gcomponent.resource.gcomponent.util.GcomponentStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new genericity.language.gcomponent.resource.gcomponent.debug.GcomponentStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public genericity.language.gcomponent.resource.gcomponent.debug.GcomponentDebugTarget getTarget() {
		return debugTarget;
	}
