...
public class GbindHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static gbind.resource.gbind.ui.GbindPositionHelper positionHelper = new gbind.resource.gbind.ui.GbindPositionHelper();
	...
	private gbind.resource.gbind.ui.GbindTokenScanner scanner;
	...
	private gbind.resource.gbind.ui.GbindEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private gbind.resource.gbind.ui.GbindOccurrence occurrence;
	private gbind.resource.gbind.ui.GbindBracketSet bracketSet;
	 ...
	 * @param editor
	 */
	public GbindHighlighting(genericity.language.gbind.IGbindTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, gbind.resource.gbind.ui.GbindColorManager colorManager, gbind.resource.gbind.ui.GbindEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = gbind.resource.gbind.ui.GbindUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new gbind.resource.gbind.ui.GbindTokenScanner(textResource, colorManager);
		occurrence = new gbind.resource.gbind.ui.GbindOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new gbind.resource.gbind.ui.GbindBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(gbind.resource.gbind.ui.GbindPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, gbind.resource.gbind.ui.GbindPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(gbind.resource.gbind.ui.GbindEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, gbind.resource.gbind.ui.GbindPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, gbind.resource.gbind.ui.GbindPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(gbind.resource.gbind.ui.GbindPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new gbind.resource.gbind.ui.GbindEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
