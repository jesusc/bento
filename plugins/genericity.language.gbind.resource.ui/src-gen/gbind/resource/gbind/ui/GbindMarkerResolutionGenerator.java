	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(genericity.language.gbind.IGbindOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof genericity.language.gbind.mopp.GbindResource) {
					genericity.language.gbind.mopp.GbindResource customResource = (genericity.language.gbind.mopp.GbindResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<genericity.language.gbind.IGbindQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final genericity.language.gbind.IGbindQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									gbind.resource.gbind.ui.GbindUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
							...
							public org.eclipse.swt.graphics.Image getImage() {
								return new gbind.resource.gbind.ui.GbindUIMetaInformation().getImageProvider().getImage(quickFix.getImageKey());
							}
			...
			gbind.resource.gbind.ui.GbindUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<genericity.language.gbind.IGbindQuickFix> getQuickFixes(genericity.language.gbind.IGbindTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					genericity.language.gbind.IGbindQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
