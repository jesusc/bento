...
public class GbindOccurrence {
	...
	private gbind.resource.gbind.ui.GbindTokenScanner tokenScanner;
	 ...
	 */
	public GbindOccurrence(genericity.language.gbind.IGbindTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, gbind.resource.gbind.ui.GbindTokenScanner tokenScanner) {
	 ...
	 */
	public void handleOccurrenceHighlighting(gbind.resource.gbind.ui.GbindBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, gbind.resource.gbind.ui.GbindPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, gbind.resource.gbind.ui.GbindPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, gbind.resource.gbind.ui.GbindPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == gbind.resource.gbind.ui.GbindPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(gbind.resource.gbind.ui.GbindOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(gbind.resource.gbind.ui.GbindOccurrence.DECLARATION_ANNOTATION_ID);
	}
