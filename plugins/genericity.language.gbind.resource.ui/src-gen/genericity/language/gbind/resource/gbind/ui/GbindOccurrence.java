...
public class GbindOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "genericity.language.gbind.resource.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "genericity.language.gbind.resource.ui.occurences.declaration";
	...
	private genericity.language.gbind.resource.gbind.ui.GbindTokenScanner tokenScanner;
	...
	private genericity.language.gbind.IGbindTextResource textResource;
	 ...
	 */
	public GbindOccurrence(genericity.language.gbind.IGbindTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, genericity.language.gbind.resource.gbind.ui.GbindTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new genericity.language.gbind.mopp.GbindMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(genericity.language.gbind.resource.gbind.ui.GbindBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, genericity.language.gbind.resource.gbind.ui.GbindPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, genericity.language.gbind.resource.gbind.ui.GbindPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, genericity.language.gbind.resource.gbind.ui.GbindPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == genericity.language.gbind.resource.gbind.ui.GbindPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(genericity.language.gbind.resource.gbind.ui.GbindOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(genericity.language.gbind.resource.gbind.ui.GbindOccurrence.DECLARATION_ANNOTATION_ID);
	}
