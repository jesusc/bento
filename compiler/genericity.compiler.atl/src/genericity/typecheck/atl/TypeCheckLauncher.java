...
import genericity.typecheck.atl.TypeCheckLauncher.ExecutionContext;
...
public class TypeCheckLauncher {
	...
	public static BasicEMFModel loadTransformationMetamodels(EMFLoader loader, String... metamodels) throws IOException {
		...
		// This is needed when there are cross references to Ecore but the
		// metamodel
		...
		if (!rs.getPackageRegistry().containsKey("platform:/plugin/org.eclipse.emf.ecore/model/Ecore.ecore"))
			rs.getPackageRegistry().put("platform:/plugin/org.eclipse.emf.ecore/model/Ecore.ecore",
	...
	public List<ErrorMessage> getMessages() {
		return new ArrayList<ErrorMessage>(context.messages);
	}
	...
	public static class CustomMethodHandler extends org.eclectic.idc.jvm.runtime.BasicMethodHandler {
		...
		public CustomMethodHandler(IModel mm, BasicEMFModel types, ModelManager m, ExecutionContext context) {
			super(m);
			this.mm = mm;
			this.types = types;
		...
		public IMethodWrapper wrap(Object o) {
			try {
				return new CustomMethodWrapper(mm, types, manager.getNamespace(o), o, context);
	...
	public static class CustomMethodWrapper extends org.eclectic.idc.jvm.runtime.BasicMethodWrapper {
		...
		public Type findCommonTypePossibleUnion(Type t1, Type t2) {
			...
			if (t == null)
				throw new RuntimeException("Incompatible types: " + t1 + " - " + t2 + " at "
		...
		public Type iteratorType(String iteratorName, Type receptorType, Type bodyType, ImmutableList restOfIterators) {
			if (iteratorName.equals("collect")) {
				...
				// This could be wiser, detecting filtering based on
				// oclIsKindOf!!
		...
		public Type operationType(Type receptorType, EObject exp, ImmutableList arguments, ImmutableList operations) {
			...
			if (receptorType instanceof TupleType) {
				throw new IllegalArgumentException("Trying to apply operation " + operationName + " to tuple "
						+ model.getFeature(object, "location"));
			}
			...
			if (receptorType instanceof Unknown) {
				System.out.println("Unknown type with operation " + operationName
						+ (receptorType.isMultivalued() ? "* " : " ") + "OclAny : " + object + " "
						+ model.getFeature(object, "location"));
				return receptorType;
			}

			/*
			 * boolean isModuleCall = false; if ( model.isKindOf(exp,
			 * "VariableExp") ) { Object vd = model.getFeature(exp,
			 * "referredVariable"); String name = (String) model.getFeature(vd,
			 * "varName"); isModuleCall = name.equals("thisModule"); }
			 */
			...
			if (receptorType instanceof PrimitiveType) {
				...
				throw new RuntimeException("No primitive operation " + operationName + " at "
						+ model.getFeature(object, "location"));
			}
		...
		public Type featureType(Type receptortype, ImmutableList helpers, ImmutableList operations) {
			...
			if (receptortype instanceof ThisModuleType) {
				if (helpers.size() < 1) {
					...
					for (Object h : helpers) {
						...
						if (ctx == null) {
							if (t != null) {
								throw new UnsupportedOperationException("Too many attributes " + featureName
										+ " found in module");
							}
			...
			if (receptortype instanceof ReflectiveClass) {
				...
				throw new UnsupportedOperationException("Feature " + featureName
						+ " not supported for reflective object");
			}

			if (!(receptortype instanceof Metaclass)) {
				throw new UnsupportedOperationException("Unsupported " + receptortype + " " + featureName + " at "
						+ model.getFeature(object, "location"));
			}
			...
			if (f == null) {
				if (helpers.size() == 1) {
					...
					if (operations.size() == 1
						...
						System.out.println("Assuming operation " + featureName + " at "
								+ model.getFeature(object, "location") + " with type " + t);
						return t;
					}
			...
			// Cache the structural feature... just as a way to have multiple
			// return values
		...
		private Type signalNoOperationError(EClass clazz, String operationName, ImmutableList operations) {
			...
			while ( it.hasNext() ) {
				EObject ctxObj = (EObject) it.next();
				Object ctx = model.getFeature(ctxObj, "context_");
				Object context = model.getFeature(ctx, "context_");
				String contextClassName = (String) model.getFeature(context, "name");

				EClassifier classifier = findMetaclass(contextClassName);
				if ( ! (classifier instanceof EClass) ) {
					continue;
				}

				EClass contextEClass = (EClass) classifier;
			
				if ( contextEClass.getEAllSuperTypes().contains(clazz) ) {
					// Conflicted operations
					if ( t != null ) {
						return throwError(errorMsg + " and " + contextEClass.getName() + ". Cannot infer from conflicting operations.", location);						
					}

					t 	     = findOclFeatureType((EObject) model.getFeature(ctxObj, "feature"));
					errorMsg = errorMsg + ", but operation found in subtype " + contextEClass.getName();
				}
			}

			return signalError(errorMsg, location, t);
		}

		private Type signalNoFeatureError(EClass clazz, String featureName) {
			String errorMessage = "No feature " + clazz.getName() + "." + featureName;
			Object location     = model.getFeature(object, "location");
			...
			for (EObject obj : allEClasses) {
				...
				if (eClass.getEAllSuperTypes().contains(clazz)) {
					...
					if (f != null) {
						lastNavigatedFeature = f;
						t =  createType(f.getEType(), f.getUpperBound() == -1 || f.getUpperBound() > 1);
						
						errorMessage = errorMessage + ", but " + eClass.getName() + " subtype found";
						break;
					}
				}
			}
		...
		private Type signalError(String message, Object location, Type t) {
			if (context.isWarningMode() && t != null) {
				System.err.println("ANALYSIS WARNING: " + message + ". Location: " + location);
				context.addMessage(new ErrorMessage(message, location.toString(), ErrorMessage.WARNING));
				return t;
			}
			return throwError(message, location);
			// throw new RuntimeException(message + ". Location: " + location);
		}

		private Type throwError(String msg, Object location) {
			context.addMessage(new ErrorMessage(msg, location.toString()));
			throw new RuntimeException(msg + " : " + location);
		}
		...
		public Type tupleAccessType(TupleType tuple, String featureName) {
			...
			throw new RuntimeException("Cannot find tuple attribute " + featureName + " at "
					+ model.getFeature(object, "location") + " within [" + attrNames + "]");
		}
		...
		private Type createDataType(EObject oclType) {
			...
			if (name.equals("BooleanType")) {
				...
				// TODO: Mark the type with a flag indicating the type of
				// collection
				...
				// TODO: This is also implemented as a rule in the
				// transformation!
				...
				while (it.hasNext()) {
					...
					types.setFeature(ta, "type",
							createTypeFromAtlTypeDefinition((EObject) model.getFeature(att, "type")));
					types.setFeature(tt, "attributes", ta);
				}

				return tt;
				// return (Type)
				// types.createObject(Unknown.class.getSimpleName()); // TODO:
				// More precise?
			}
		...
		public Type createType(EClassifier classifier, boolean multivalued) {
			if (classifier instanceof EEnum) {
				...
				else if (classifier.getName().endsWith("UnlimitedNatural")
						|| classifier.getName().startsWith("Integer"))
					return (Type) types.createObject(IntegerType.class.getSimpleName());

				else
					throw new UnsupportedOperationException("Type " + classifier.getName() + " not supported");
		...
		private Type mapOperationType(MapType mt, String operationName, ImmutableList arguments) {
			if (operationName.equals("including")) {
				...
				// TODO: This is probably not perfectly fince, since one unknown
				// types leads to unknown...
			...
			throw new RuntimeException("No operation " + operationName + " for Map: "
					+ model.getFeature(object, "location"));
		}
		...
		public Type collectionOperationType(Type t, ImmutableList arguments) {
			...
			String operationName = (String) model.getFeature(object, "operationName");
			if (operationName.equals("first") || operationName.equals("at")) {
			...
			} else if (operationName.equals("including") || operationName.equals("append")
					|| operationName.equals("union")) {
				if ( arguments.size() != 1 ) {
					throw new IllegalArgumentException("Expecting one argument for " + operationName);
				}				
				Type ct = findCommonTypeAux(t, (Type) arguments.first(), true);
				if (ct == null) {
					throw new UnsupportedOperationException("Cannot find common type: " + operationName + " " + t
	...
	public static class ErrorMessage {
		...
		public ErrorMessage(String msg, String location) {
			this(msg, location, ERROR);
		}

		public ErrorMessage(String msg, String location, int type) {
			this.msg = msg;
		...
		public boolean isWarning() { return type == WARNING; }
