...
public class GbindBracketSet {
	...
	private java.util.ArrayList<genericity.language.gbind.IGbindBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements genericity.language.gbind.IGbindBracketPair {
	 ...
	 */
	private class ClosingListener implements genericity.language.gbind.resource.gbind.ui.IGbindBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				genericity.language.gbind.IGbindBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public GbindBracketSet(genericity.language.gbind.resource.gbind.ui.GbindEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new genericity.language.gbind.mopp.GbindMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<genericity.language.gbind.IGbindBracketPair>();
	 ...
	 */
	public genericity.language.gbind.IGbindBracketPair getBracketPair(String opening, String closing) {
		for (genericity.language.gbind.IGbindBracketPair bracketPair : bracketPairs) {
	...
	public genericity.language.gbind.IGbindBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(genericity.language.gbind.IGbindBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + genericity.language.gbind.resource.gbind.ui.GbindPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public genericity.language.gbind.IGbindBracketPair remove(String opening, String closing) {
		for (genericity.language.gbind.IGbindBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<genericity.language.gbind.IGbindBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, genericity.language.gbind.resource.gbind.ui.GbindPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, genericity.language.gbind.resource.gbind.ui.GbindPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
