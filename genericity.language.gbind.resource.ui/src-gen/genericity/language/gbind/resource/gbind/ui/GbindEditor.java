 ...
 * <p>
 * This editor has id
 * <code>genericity.language.gbind.resource.gbind.ui.GbindEditor</code>
 * The editor's context menu has id
 * <code>genericity.language.gbind.resource.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>genericity.language.gbind.resource.EditorRuler</code>.
 * </p>
 */
public class GbindEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, genericity.language.gbind.IGbindResourceProvider, genericity.language.gbind.resource.gbind.ui.IGbindBracketHandlerProvider, genericity.language.gbind.resource.gbind.ui.IGbindAnnotationModelProvider {
	
	private genericity.language.gbind.resource.gbind.ui.GbindHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private genericity.language.gbind.resource.gbind.ui.GbindCodeFoldingManager codeFoldingManager;
	private genericity.language.gbind.resource.gbind.ui.GbindBackgroundParsingStrategy bgParsingStrategy = new genericity.language.gbind.resource.gbind.ui.GbindBackgroundParsingStrategy();
	private java.util.Collection<genericity.language.gbind.IGbindBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<genericity.language.gbind.IGbindBackgroundParsingListener>();
	private genericity.language.gbind.resource.gbind.ui.GbindColorManager colorManager = new genericity.language.gbind.resource.gbind.ui.GbindColorManager();
	private genericity.language.gbind.resource.gbind.ui.GbindOutlinePage outlinePage;
	...
	private genericity.language.gbind.resource.gbind.ui.GbindPropertySheetPage propertySheetPage;
	...
	private genericity.language.gbind.resource.gbind.ui.IGbindBracketHandler bracketHandler;
	...
	public GbindEditor() {
		super();
		setSourceViewerConfiguration(new genericity.language.gbind.resource.gbind.ui.GbindSourceViewerConfiguration(this, this, this, colorManager));
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								genericity.language.gbind.IGbindTextResource currentResource = getResource();
				...
				genericity.language.gbind.resource.gbind.ui.GbindUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("genericity.language.gbind.resource.EditorContext");
		setRulerContextMenuId("genericity.language.gbind.resource.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new genericity.language.gbind.resource.gbind.ui.GbindHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new genericity.language.gbind.resource.gbind.ui.GbindCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		genericity.language.gbind.mopp.GbindNature.activate(inputFile.getProject());
		...
		genericity.language.gbind.IGbindTextResource loadedResource = (genericity.language.gbind.IGbindTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				genericity.language.gbind.IGbindTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof genericity.language.gbind.IGbindTextResource) {
					setResource((genericity.language.gbind.IGbindTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					genericity.language.gbind.resource.gbind.ui.GbindUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the GbindEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				genericity.language.gbind.resource.gbind.ui.GbindUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(genericity.language.gbind.IGbindTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new genericity.language.gbind.resource.gbind.ui.GbindOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new genericity.language.gbind.resource.gbind.ui.GbindPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			genericity.language.gbind.IGbindTextResource textResource = (genericity.language.gbind.IGbindTextResource) element.eResource();
			genericity.language.gbind.IGbindLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			genericity.language.gbind.IGbindTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				genericity.language.gbind.IGbindTextToken token = lexer.getNextToken();
			...
			genericity.language.gbind.resource.gbind.ui.GbindUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	public void addBackgroundParsingListener(genericity.language.gbind.IGbindBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (genericity.language.gbind.IGbindBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public genericity.language.gbind.resource.gbind.ui.IGbindBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(genericity.language.gbind.resource.gbind.ui.IGbindBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof genericity.language.gbind.IGbindTextResource)) {
					return false;
				}
				genericity.language.gbind.IGbindTextResource textResource = (genericity.language.gbind.IGbindTextResource) resource;
				genericity.language.gbind.IGbindLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
