 ...
 *******************************************************************************/
package bento.componetization.atl.metrics.files;
...
/**
 * Entry point of the 'Ecore2measure' transformation module.
 */
public class Ecore2measure {
	...
	public static void main(String[] args) {
		try {
			if (args.length < 2) {
				...
				Ecore2measure runner = new Ecore2measure();
				runner.loadModels(args[0]);
				runner.doEcore2measure(new NullProgressMonitor());
	 ...
	 * @generated
	 */
	public Object doEcore2measure(IProgressMonitor monitor) throws ATLCoreException, IOException, ATLExecutionException {
		ILauncher launcher = new EMFVMLauncher();
		Map<String, Object> launcherOptions = getOptions();
		launcher.initialize(launcherOptions);
		launcher.addInModel(inModel, "IN", "ECORE");
		launcher.addOutModel(outModel, "OUT", "Measure");
	...
	protected InputStream[] getModulesList() throws IOException {
		...
		String modulesList = properties.getProperty("Ecore2measure.modules");
		if (modulesList != null) {
	...
	protected String getMetamodelUri(String metamodelName) {
		return properties.getProperty("Ecore2measure.metamodels." + metamodelName);
	}
	...
	protected InputStream getLibraryAsStream(String libraryName) throws IOException {
		return getFileURL(properties.getProperty("Ecore2measure.libraries." + libraryName)).openStream();
	}
	...
	protected Map<String, Object> getOptions() {
		...
		for (Entry<Object, Object> entry : properties.entrySet()) {
			if (entry.getKey().toString().startsWith("Ecore2measure.options.")) {
				options.put(entry.getKey().toString().replaceFirst("Ecore2measure.options.", ""), 
	...
	protected static URL getFileURL(String fileName) throws IOException {
		final URL fileURL;
		if (isEclipseRunning()) {
			URL resourceURL = Ecore2measure.class.getResource(fileName);
			if (resourceURL != null) {
			...
			fileURL = Ecore2measure.class.getResource(fileName);
		}
		if (fileURL == null) {
